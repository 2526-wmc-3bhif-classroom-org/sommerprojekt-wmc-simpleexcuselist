...
function currentSemesterRange(): { from: string; to: string; label: string } {
  ...
  if (month >= 9)
    return {
      from: `${year}-09-01`,
      to: `${year + 1}-01-31`,
      label: `WS ${year}/${String(year + 1).slice(2)}`,
    }
  if (month <= 2)
    return {
      from: `${year - 1}-09-01`,
      to: `${year}-01-31`,
      label: `WS ${year - 1}/${String(year).slice(2)}`,
    }
...
const isSemesterDefault = computed(
  () => dateFrom.value === SEMESTER.from && dateTo.value === SEMESTER.to,
)
...
<template>
  <div
    class="min-h-screen w-full bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex flex-col font-sans transition-colors duration-300"
  >
    <!-- Top Navigation Header -->
    <header
      class="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between px-6 py-4 transition-colors duration-300"
    >
      <div class="flex items-center gap-3">
        <div
          class="bg-primary/10 text-primary p-2 rounded-xl font-bold tracking-widest text-xs uppercase dark:bg-primary/20"
        >
          Excuses
        </div>
      ...
      <div class="flex items-center gap-4">
        <!-- Theme Toggle -->
        <div
          class="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 p-2 rounded-lg transition-colors"
        >
          <LightingMode />
        </div>
        <button
          @click="logout"
          class="bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 px-4 py-2 rounded-xl transition text-sm font-bold"
        >
          Abmelden
        </button>
      </div>
    ...
    <main class="flex-1 w-full px-6 md:px-12 py-10 space-y-6">
      ...
      <div
        class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4"
      >
        <div>
          ...
          <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">
            ...
            <template v-else> Stundenplan- und Fachstatistiken deiner Abwesenheiten. </template>
          </p>
          <div v-if="!showAnalytics" class="flex items-center gap-3 mt-3 flex-wrap">
            <!-- Gesamt -->
            <div
              class="flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 shadow-sm min-w-[120px]"
            >
              <span
            ...
            <!-- Davon offen -->
            <div
              class="flex flex-col bg-white dark:bg-slate-900 border border-yellow-200 dark:border-yellow-900/50 rounded-xl px-4 py-2 shadow-sm min-w-[110px]"
            >
              <span class="text-xs font-semibold text-yellow-500 uppercase tracking-wider"
                >Davon offen</span
              >
              <div class="flex items-baseline gap-1.5 mt-0.5">
                <span
                  class="text-xl font-black text-yellow-500 dark:text-yellow-400 tabular-nums"
                  >{{ openCount }}</span
                >
            ...
            <!-- Davon entschuldigt -->
            <div
              class="flex flex-col bg-white dark:bg-slate-900 border border-emerald-200 dark:border-emerald-900/50 rounded-xl px-4 py-2 shadow-sm min-w-[110px]"
            >
              <span class="text-xs font-semibold text-emerald-500 uppercase tracking-wider"
                >Davon entschuldigt</span
              >
              <div class="flex items-baseline gap-1.5 mt-0.5">
                <span
                  class="text-xl font-black text-emerald-500 dark:text-emerald-400 tabular-nums"
                  >{{ excusedCount }}</span
                >
            ...
            <div
              class="flex flex-col bg-white dark:bg-slate-900 border border-orange-200 dark:border-orange-900/50 rounded-xl px-4 py-2 shadow-sm min-w-[110px]"
            >
              <span class="text-xs font-semibold text-orange-400 uppercase tracking-wider"
                >Davon nicht entschuldigt</span
              >
              <div class="flex items-baseline gap-1.5 mt-0.5">
                <span
                  class="text-xl font-black text-orange-500 dark:text-orange-400 tabular-nums"
                  >{{ openAbsenceCount }}</span
                >
        ...
        <div class="flex items-center gap-3 flex-wrap">
          ...
          <div
            class="flex space-x-2 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-1 shadow-sm transition-colors duration-300"
          >
            <button
              @click="showAnalytics = false"
              :class="[
                'px-4 py-1.5 text-xs font-bold rounded-xl cursor-pointer transition-colors',
                !showAnalytics
          ...
          <button
            v-if="!showAnalytics"
            @click="fetchAbsences()"
            class="flex items-center gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 font-bold px-3.5 py-2 text-xs rounded-xl shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition cursor-pointer"
          >
            <svg
              class="w-3.5 h-3.5 text-slate-450"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      <div
        class="flex items-center gap-2 flex-wrap bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-2.5 shadow-sm transition-colors"
      >
        <svg
        ...
        <span
          class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mr-1"
          >Zeitraum</span
        >

        <!-- Semester chip -->
        <span
          class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold border transition-colors"
          :class="
            isSemesterDefault
        ...
        >
          <span
            ...
            :style="{ background: isSemesterDefault ? 'var(--color-primary, #7C3AED)' : '#9ca3af' }"
          ></span>
          {{ SEMESTER.label }}
        </span>
      ...
      <div class="space-y-4">
        ...
        <div v-if="showAnalytics" class="space-y-6">
          <div
            ...
            class="flex justify-center py-16 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-900 transition-colors duration-300"
          >
            <div
              class="w-7 h-7 border-2 border-slate-200 border-t-primary rounded-full animate-spin"
            ></div>
          </div>
          <div
            v-else-if="analytics.stats.length === 0"
            class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-900 p-10 flex flex-col items-center text-center transition-colors duration-300"
          >
            <div
              class="w-12 h-12 bg-emerald-50 dark:bg-emerald-950/20 rounded-full flex items-center justify-center mb-3 border border-emerald-100 dark:border-emerald-900/35 shadow-sm"
            >
              <svg
                class="w-6 h-6 text-emerald-505"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <h3 class="text-base font-bold text-slate-955 dark:text-white tracking-tight">
              Keine Fehlstunden in der Analyse
            </h3>
          ...
          <div v-else class="space-y-6">
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <!-- Heatmap -->
              <div
                class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-900 rounded-2xl p-6 shadow-sm transition-colors overflow-hidden"
              >
                <h3
              ...
              <!-- Pie Chart -->
              <div
                class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-900 rounded-2xl p-6 shadow-sm transition-colors flex flex-col overflow-hidden"
              >
                <h3
                  class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-4"
                >
                  Versäumte Stunden nach Fach
                </h3>
            ...
            <div
              class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-900 rounded-2xl shadow-sm overflow-hidden transition-colors"
            >
              ...
              <div class="overflow-x-auto">
                <table class="w-full text-sm text-left border-collapse">
                  <thead
                    class="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider"
                  >
                    <tr>
                      ...
                      <td class="px-6 py-3.5 text-right">
                        <span
                          :class="[
                            ...
                            severityClass(s.missedLessons),
                          ]"
                        >
                          {{ s.missedLessons }}
                        </span>
                      </td>
                      <td class="px-6 py-3.5 text-right text-slate-500 dark:text-slate-400">
                        {{ s.totalLessons ?? '—' }}
                      </td>
                      <td class="px-6 py-3.5 text-right">
                        <span
        ...
        <div v-else class="space-y-4">
          ...
          <!-- Filter Bar -->
          <div
            class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-900 p-4 rounded-2xl shadow-sm transition-colors duration-300 space-y-3"
          >
            <!-- Search -->
                ...
                v-for="tab in [
                  {
                    key: 'all',
                    label: 'Alle',
                    cls: 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200',
                  },
                  {
                    key: 'open',
                    label: 'Offen',
                    cls: 'bg-orange-50 dark:bg-orange-950/30 text-orange-600 dark:text-orange-400',
                  },
                  {
                    key: 'pending',
                    label: 'Abgeschickt',
                    cls: 'bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400',
                  },
                  {
                    key: 'signed',
                    label: 'Unterschrieben',
                    cls: 'bg-violet-50 dark:bg-violet-950/30 text-violet-600 dark:text-violet-400',
                  },
                  {
                    key: 'excused',
                    label: 'Entschuldigt',
                    cls: 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400',
                  },
                  {
                    key: 'unexcused',
                    label: 'Nicht entschuldigt',
                    cls: 'bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400',
                  },
                ]"
                :key="tab.key"
                ...
                <span class="ml-1 opacity-60 tabular-nums">
                  ({{
                    tab.key === 'all'
                      ? absences.length
                      : tab.key === 'excused'
                        ? absences.filter((a) => a.isExcusedUntis || a.status === 'excused').length
          ...
          <div
            class="flex gap-1 p-1 mb-6 bg-slate-100 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-900 w-full sm:w-fit"
          >
            <button
          ...
          <div
            v-else-if="error"
            class="p-6 bg-red-500/10 border border-red-500/20 text-red-700 dark:text-red-400 rounded-2xl"
          >
          ...
          <div
            v-else-if="filteredAbsences.length === 0"
            class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-900 p-10 flex flex-col items-center text-center transition-colors duration-300"
          >
            <div
              class="w-12 h-12 bg-emerald-50 dark:bg-emerald-950/20 rounded-full flex items-center justify-center mb-3 border border-emerald-100 dark:border-emerald-900/35 shadow-sm"
          ...
          <div v-else class="space-y-3">
            ...
            >
              ...
              <div class="flex items-center gap-3">
                <div
                  class="w-9 h-9 bg-slate-100 dark:bg-slate-850 rounded-xl flex items-center justify-center flex-shrink-0 text-slate-550 dark:text-slate-400"
                >
                ...
                <div class="flex flex-col">
                  ...
                  <span class="text-xs text-slate-500 dark:text-slate-400 mt-0.5"
                    ...
                    {{ formatTime(absence.endTime) }} Uhr</span
                  >
                </div>
              ...
              <div
                class="flex items-center justify-end border-t sm:border-0 border-slate-100 dark:border-slate-800/80 pt-2.5 sm:pt-0"
              >
                <button
    ...
    >
      ...
      >
        <div
          class="w-16 h-16 rounded-full bg-emerald-50 dark:bg-emerald-950/20 flex items-center justify-center border border-emerald-100 dark:border-emerald-900/35 mb-4"
        >
          <svg
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-2 max-w-xs leading-relaxed">
          Dein Entschuldigungsantrag wurde gespeichert und deinem Elternteil zur Freigabe
          übermittelt.
        </p>
        <button
          @click="closeModal"
          class="mt-8 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 px-6 py-2.5 rounded-xl text-sm font-bold transition"
        >
          Schließen
        </button>
      </div>
      ...
      <div v-else class="flex-1 flex flex-col h-full bg-white dark:bg-slate-900">
        <!-- Drawer Header -->
        <div
          class="flex items-center justify-between px-6 py-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/60 transition-colors"
        >
          <div>
            <h3 class="font-black text-lg text-slate-950 dark:text-white tracking-tight">
              Absenz entschuldigen
            </h3>
            <span
              v-if="activeAbsence"
              class="block text-xs font-semibold text-slate-400 dark:text-slate-500 mt-1"
            >
              Am {{ formatDate(activeAbsence.date) }} · {{ formatTime(activeAbsence.startTime) }} –
              {{ formatTime(activeAbsence.endTime) }} Uhr
            </span>
          </div>
          <button
            @click="closeModal"
        ...
        <div class="flex-1 p-6 overflow-y-auto space-y-6">
          ...
          <div>
            ...
            >
              <svg
                class="w-6 h-6 mx-auto text-slate-450 dark:text-slate-500 mb-2"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <p class="text-sm text-slate-600 dark:text-slate-400">
                Datei hierher ziehen oder <span class="text-primary font-bold">durchsuchen</span>
              </p>
              <span class="text-[10px] text-slate-400 block mt-1"
                >Unterstützt PDF, JPG, PNG (max. 10MB)</span
              >
            </div>
            <input
              ref="fileInputRef"
            ...
            <div v-if="excuseFiles.length > 0" class="mt-4 space-y-2 max-h-48 overflow-y-auto">
              ...
              >
                ...
                <span
                  class="text-xs text-slate-700 dark:text-slate-300 flex-1 truncate font-semibold"
                  >{{ file.name }}</span
                >
                <button
                  @click="removeFile(i)"
                  class="text-slate-400 hover:text-red-500 transition px-1.5 cursor-pointer"
                >
        ...
        <div
          class="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex justify-end gap-3 transition-colors"
        >
          <button
            @click="closeModal"
            class="px-4 py-2.5 bg-white dark:bg-slate-850 border border-slate-300 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 shadow-sm transition"
          >
            Abbrechen
          </button>
          <button
